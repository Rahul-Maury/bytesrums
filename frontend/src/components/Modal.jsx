...
const errorReducer = (state, action) => {
  switch (action.type) {
    ...
    case "REMOVE_YEAR_ERROR":return{
       ...state,
        yearErrMsg:" ",
        isYearErr:false
    }
    case "REMOVE_GENRE_ERROR" :return {
       ...state,
        genreErrMsg:"",
        isGenreErr:false
    }
    case "REMOVE_AUTHOR_ERROR": return{
       ...state,
        authorErrMsg:"",
        isAuthorErr:false
    }
...
const Modal = (props) => {
  const [formData, dispatch] = useReducer(formReducer, props.book?props.book:formInitialData);
  ...
  const submitHandler = async () => {
    ...
    try{
      const res=await axios.put(`http://localhost:8000/api/books/${props.book._id}`,formData)
      props.onRefresh();
    }catch(e){
      console.log(e)
    }
